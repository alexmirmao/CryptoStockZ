...
async function getLottery(lotteryAddress) {
  let res = await contract.methods.getLottery().call();
  return { message: res };
}
...
async function createLottery(privateKey, lotteryData) {
  ...
  let res = await contract.methods.createLottery(
      lotteryData.maxNumberParticipants, 
      lotteryData.participationPrice, 
      lotteryData.prize, 
      lotteryData.participationPot)
    .send(
    {
      from: user.address,
      gasLimit: gasLimit,
...
async function setLotteryStorage(privateKey, lotteryStorageAddr) {
  ...
  console.log(lotteryStorageAddr);
  return { message: res };
}
